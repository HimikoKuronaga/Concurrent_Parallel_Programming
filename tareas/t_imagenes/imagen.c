...
void shortRgbToGray( unsigned char *imgRGB, uint32_t width, uint32_t height ){
	unsigned char nivel;
	register int indiceRGB, indiceGRAY;
	for( indiceRGB = 0, indiceGRAY = 0; indiceGRAY < width * height; indiceGRAY++, indiceRGB += 3){
		nivel = (30*imgRGB[indiceRGB] + 59*imgRGB[indiceRGB + 1] + 11*imgRGB[indiceRGB + 2]) / 100;
		imgRGB[indiceRGB] = nivel;
		imgRGB[indiceRGB + 1] = nivel;
		imgRGB[indiceRGB + 2] = nivel;
	}
}

void grayToRgb( unsigned char *imgRGB, unsigned char *imgGRAY, uint32_t width, uint32_t height ){
	register int indiceRGB, indiceGRAY;
	for( indiceRGB = 0, indiceGRAY = 0; indiceGRAY < (width * height); indiceGRAY++, indiceRGB += 3){
